...
class AgentDQN(BaseAgent):
    ...
    def compute_loss(self, batch_vars):
        ...
        num_diseases = 7
        batch_state_flag = self.get_sym_flag(batch_state[:,(2*self.act_cardinality+num_diseases+1):(2*self.act_cardinality+self.slot_cardinality)])
    ...
    def run_policy(self, rep, state):
        ...
        if np.random.random() < self.epsilon:
            ...
            if self.warm_start == 1:
                ...
                num_diseases = 7
                sym_flag = self.get_sym_flag(rep[:, (2*self.act_cardinality+num_diseases + 1):(2*self.act_cardinality+self.slot_cardinality)])
