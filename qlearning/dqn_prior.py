...
class Knowledge_Graph_Reasoning(nn.Module):
    def __init__(self, num_actions, dise_start, act_cardinality, slot_cardinality, dise_sym_mat, sym_dise_mat, sym_prio):
        ...
        self.num_diseases = self.dise_sym_mat.shape[0]

    def forward(self, state):
        current_slots_rep = state[:, (2*self.act_cardinality + self.num_diseases + 1):(2*self.act_cardinality + self.slot_cardinality)]
